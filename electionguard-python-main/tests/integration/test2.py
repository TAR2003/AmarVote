...
def create_election_manifest() -> Manifest:
    ...
    contests: List[Contest] = [
        ...
        Contest(
            object_id="contest-2",
            sequence_order=1,
            electoral_district_id="county-2",
            vote_variation="one_of_m",
            name="County Commissioner",
            ballot_selections=[
                SelectionDescription(
                    object_id="contest-2-candidate-4",
                    candidate_id="candidate-4",
                    sequence_order=0,
                ),
                SelectionDescription(
                    object_id="contest-2-candidate-5",
                    candidate_id="candidate-5",
                    sequence_order=1,
                ),
                SelectionDescription(
                    object_id="contest-2-candidate-6",
        ...
        Contest(
            object_id="contest-3",
            sequence_order=2,
            electoral_district_id="state-1",
            vote_variation="one_of_m",
            name="Governor",
            ballot_selections=[
                SelectionDescription(
                    object_id="contest-3-candidate-1",
                    candidate_id="candidate-1",
                    sequence_order=0,
                ),
                SelectionDescription(
                    object_id="contest-3-candidate-2",
                    candidate_id="candidate-2",
                    sequence_order=1,
                ),
                SelectionDescription(
                    object_id="contest-3-candidate-3",
                    candidate_id="candidate-3",
                    sequence_order=2,
                ),
            ],
    ...
    ballot_styles: List[BallotStyle] = [
        BallotStyle(
            ...
            geopolitical_unit_ids=["county-1", "state-1"],
            party_ids=None,
            image_uri=None,
        ),
        BallotStyle(
            object_id="ballot-style-2",
            geopolitical_unit_ids=["county-2", "state-1"],
            party_ids=None,
            image_uri=None,
        ),
    ]
...
def create_plaintext_ballots(manifest: Manifest, ballot_count: int = 20) -> List[PlaintextBallot]:
    ...
    for i in range(ballot_count):
        ...
        ballot_style = random.choice(ballot_styles)
        ...
        for contest in available_contests:
            ...
            if contest.vote_variation == "one_of_m":
                # Choose one selection for one_of_m contests
...
def run_election_demo():
    ...
    #     print("❌ Error: The election manifest is not valid!")
    #     return
    
    print("✅ Election manifest is valid")
    ...
    device = EncryptionDevice(f"device-{uuid.uuid4()}")
    ...
    for contest in manifest.contests:
        ...
        if contest_results:
            for selection in contest.ballot_selections:
                ...
                decrypted_vote_count = contest_results.ballot_selections[selection.object_id].tally
                ...
                for contest in original_ballot.contests:
                    ...
                    if spoiled_contest:
                        for selection in contest.ballot_selections:
                            ...
                            decrypted_vote = spoiled_contest.ballot_selections[selection.object_id].tally
